...
public class PetriNetView extends Observable implements Cloneable, IObserver, Serializable, Observer {
    ...
    private Parameter createParameter(Element inputDefinitionElement) {
        ...
        return new RateParameter(nameTemp, Double.parseDouble(expressionTemp), positionXInput, positionYInput);
    }
    ...
    private TransitionView createTransition(Element element) {
        ...
        if (nameTimed.length() == 0) {
            ...
            timedTransition = nameTimed.length() != 5;
        }

        infiniteServer = !(nameInfiniteServer.length() == 0 || nameInfiniteServer.length() == 5);

        if (positionXTempStorage.length() > 0) {
            positionXInput = Double.valueOf(positionXTempStorage).doubleValue() * (1) + (1);
        }
        if (positionYTempStorage.length() > 0) {
            positionYInput = Double.valueOf(positionYTempStorage).doubleValue() * (1) + (1);
        }
        ...
        if (parameterTempStorage.length() > 0) {
            if (existsRateParameter(parameterTempStorage)) {
                for (RateParameter _rateParameter : _rateParameters) {
                    if (parameterTempStorage.equals(_rateParameter.getName())) {
    ...
    private PlaceView createPlace(Element element) {
        ...
        PlaceView placeView =
                new PlaceView(positionXInput, positionYInput, idInput, nameInput, nameOffsetXInput, nameOffsetYInput,
                        initialMarkingViewInput, markingOffsetXInput, markingOffsetYInput, capacityInput);
        for (MarkingView markingView : initialMarkingViewInput) {
            if (markingView.getCurrentMarking() > 0) {
                lockTokenClass(markingView.getToken().getID());
            }
        }
        return placeView;
        ...
        for (int i = 0; i < transitionCount; i++) {
            ...
            for (int j = 0; j < placeCount; j++) {
                ...
                for (MarkingView m : markings[j]) {
                    ...
                    if (allTokenClassesEnabled && (_capacityMatrix[j] > 0) && (totalMarkings + totalCPlus - totalCMinus
                            > _capacityMatrix[j])) { // firing this transition would break a capacity
                        // restriction so the transition is not enabled
                        allTokenClassesEnabled = false;
                    }
        ...
        if (inscriptionTempStorage.length() > 0) {
            ...
            if (stringArray.length == 1) {
                ...
                MarkingView markingView =
                        new MarkingView(getActiveTokenView(), stringArray[0]);// Integer.valueOf(stringArray[0])+"");
                Marking marking = new Marking(getActiveTokenView().getModel(),
                        stringArray[0]);//Integer.valueOf(stringArray[0])+"");
                ...
                while (i < stringArray.length) {
                    ...
                    MarkingView markingView = new MarkingView(this.getTokenClassFromID(stringArray[i]),
                            stringArray[i + 1]);//Integer.valueOf(stringArray[i + 1])+"");
                    Marking marking = new Marking(getTokenClassFromID(stringArray[i]).getModel(),
                            stringArray[i + 1]);//Integer.valueOf(stringArray[i + 1])+"");
                    weightInput.add(markingView);
                    weightModel.add(marking);
                    i += 2;
        ...
        else if(type.equals("virtual"))
        {
            ...
            tempArcView =
                    new NormalArcView((double) aStartx, (double) aStarty, (double) aEndx, (double) aEndy, sourceIn,
                            ...
                            new NormalArc(sourceIn.getModel(), targetIn.getModel()));//, weightModel));
        }
            ...
            for (int i = 0; i < nodelist.getLength(); i++) {
                ...
                if (node instanceof Element) {
                    ...
                    if ("arcpath".equals(element.getNodeName())) {
                        ...
                        tempArcView.getArcPath().addPoint(arcPointX, arcPointY, arcPointType);
                    }
                }
            }
        }
